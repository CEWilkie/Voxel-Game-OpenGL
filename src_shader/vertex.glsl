...
layout(location = 2) in vec3 modelVertex;
...
void main() {
    ...
    vec3 originVertex = modelVertex - vec3(0.5, 0.5, 0.5);
    ...
    gl_Position = vec4(rotatedOriginPos + chunkPosition, 1.0f);
    ...
    v_vertexTextureCoord = vec2(uVertexTextureCoordOffset.x + textureCoord.x,
                                uVertexTextureCoordOffset.y + textureCoord.y) / 16.0f;
